...
class Covariance(Spectrum4d):
    ...
    def __init__(self, s):
        ...
        super(Covariance, self).__init__(dims = s.dims, 
    ...
    def get_cov(self, m):
        ...
        for dim_0 in range(d):
            for dim_1 in range(d):
                integral = 0
                # calculate integral
    ...
    def cosine_cov(self, theta_in):
        ...
        for i in range(self.sampling_rate):
            for j in range(self.sampling_rate):
                for k in range(self.sampling_rate):
                    for l in range(self.sampling_rate):
                        cos_light_field[i, j, k, l] = max(0, np.cos(k - self.radius + theta_in))
    ...
    def bsdf_cov(self):
        ...
        # get bsdf spectrum cov in 4d
        bsdf_dist = self.bsdf[self.radius]
        var = bsdf_dist.var()
        bsdf_dist_cov = np.zeros(self.cov.shape)
        bsdf_dist_cov[2, 2] = var
        print('bsdf_dist_cov: \n{}'.format(bsdf_dist_cov))

        # We approximate it using the formula for zero-centered
        # Gaussians multiplication. When we multiply two zero-centered Gaussians, 
        # the resulting distribution is a Gaussian whose inverse covariance matrix is 
        # the sum of input inverse covariance matrices.
        self.cov = pinv(pinv(self.cov) + pinv(bsdf_dist_cov))
